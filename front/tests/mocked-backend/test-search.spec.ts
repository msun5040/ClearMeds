...
test("after I search without loading, i get a message", async ({page,}) => {
  ...
  await page.route(
    "http://localhost:4000/searchcsv?target=providence&identifier=undefined&header=undefined",
    (route) => {
      route.fulfill({
        body: JSON.stringify({
          result: "failure",
...
test("search after loading no headers", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=withoutHeaders.csv",
    (route) => {
      route.fulfill({
...
test("search after loading w/ headers", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=withHeaders.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("search with column number", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/searchcsv?target=providence&identifier=1&header=undefined",
    (route) => {
      route.fulfill({
        body: JSON.stringify({
          result: "success",
...
test("search with column name", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/searchcsv?target=providence&identifier=city&header=undefined",
    (route) => {
      route.fulfill({
        body: JSON.stringify({
          result: "success",
...
test("testing the loading of a file -> view -> search", async ({ page }) => {
  ...
  /////LOAD FILE 1
  await page.route(
    "http://localhost:4000/loadcsv?filepath=noHeadersCSV",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
  ...
  /////VIEW FILE 1
  await page.route("http://localhost:4000/viewcsv", (route) => {
    route.fulfill({
      body: JSON.stringify({
        result: "success",
...
test("testing mode switch for search", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/searchcsv?target=providence&identifier=undefined&identifier=undefined&header=undefined",
    (route) => {
      route.fulfill({
        body: JSON.stringify({
          result: "success",
