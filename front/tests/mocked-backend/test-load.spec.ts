...
test("correct response for a file that exists", async ({ page }) => {
  ...
  await page.route("http://localhost:4000/loadcsv?filepath=data/ri_income.csv",
    (route) => {
      route.fulfill({
...
test("correct response for a file that does not exist", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=blah.blah.blah",
    (route) => {
      route.fulfill({
...
test("correct response for load_file command without a specified path", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=",
    (route) => {
      route.fulfill({
...
test("file path updates after second load", async ({ page }) => {
  ...
  /////LOAD FILE 2
  await page.route(
    "http://localhost:4000/loadcsv?filepath=emptyCSV",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("mode changes works correctly with load", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=goodpath",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=goodpath2",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
