...
test("correct rendering of a file without headers", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=backend/data/ten-star.csv",
    (route) => {
      route.fulfill({
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file backend/data/ten-star.csv");
  await page.getByRole("button", { name: "Submit" }).click();
...
test("correct rendering of a file that is empty", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=empty.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("correct rendering of a file with one row", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=oneRow.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("correct rendering of a file with one col", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=oneCol.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("correct rendering of a file with holes", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=holes.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
...
test("correct rendering of a file in verbose and brief mode", async ({ page }) => {
  ...
  await page.route(
    "http://localhost:4000/loadcsv?filepath=reg.csv",
    (route) => {
      route.fulfill({
        body: JSON.stringify({ result: "success" }),
      });
